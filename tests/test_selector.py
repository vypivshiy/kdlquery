...
def _first_args(results):
    return [n.get_arg(0) for n in results]


def _node_reprs(results):
    parts = []
    for n in results:
        r = n.name
        if n.args:
...
class TestNodeSelector:
    def test_app(self, doc):
        r = doc.select("app")
        assert _names(r) == ["app"]

    def test_server(self, doc):
        r = doc.select("server")
        assert _first_args(r) == ["primary", "replica"]

    def test_wildcard_root(self, doc):
        r = doc.select("*:root")
        assert _names(r) == ["app"]
...
class TestPropertyBasic:
    def test_server_tls_exists(self, doc):
        r = doc.select("server[tls]")
        assert _first_args(r) == ["primary", "replica"]

    def test_server_tls_true(self, doc):
        r = doc.select("server[tls=#true]")
        assert _first_args(r) == ["primary"]

    def test_server_tls_false(self, doc):
        r = doc.select("server[tls=#false]")
        assert _first_args(r) == ["replica"]

    def test_server_port_8080(self, doc):
        r = doc.select("server[port=8080]")
        assert _first_args(r) == ["primary"]

    def test_route_auth_true(self, doc):
        r = doc.select("route[auth=#true]")
        assert _first_args(r) == ["GET", "POST"]

    def test_route_handler_starts_users(self, doc):
        r = doc.select('route[handler^="users"]')
        assert _first_args(r) == ["GET", "POST"]

    def test_route_handler_ends_check(self, doc):
        r = doc.select('route[handler$="check"]')
        assert _first_args(r) == ["GET"]

    def test_route_handler_contains_serve(self, doc):
        r = doc.select('route[handler~="serve"]')
        assert _first_args(r) == ["GET"]

    def test_app_version_exists(self, doc):
        r = doc.select("app[version]")
        assert _names(r) == ["app"]

    def test_app_version_value(self, doc):
...
class TestPropertyTypeAnnotation:
    def test_u16_port_any_node(self, doc):
        r = doc.select("[(u16)port]")
        assert _names(r) == ["backend"]

    def test_backend_u16_port(self, doc):
        r = doc.select("backend[(u16)port]")
        assert _names(r) == ["backend"]

    def test_backend_u16_port_value(self, doc):
        r = doc.select("backend[(u16)port=6379]")
        assert _names(r) == ["backend"]

    def test_u32_max_conn_any(self, doc):
        r = doc.select("[(u32)max-conn]")
        assert _names(r) == ["limits"]

    def test_limits_u32_max_conn(self, doc):
        r = doc.select("limits[(u32)max-conn]")
        assert _names(r) == ["limits"]

    def test_limits_u32_max_conn_value(self, doc):
        r = doc.select("limits[(u32)max-conn=1000]")
        assert _names(r) == ["limits"]

    def test_limits_u32_max_req_value(self, doc):
        r = doc.select("limits[(u32)max-req=500]")
        assert _names(r) == ["limits"]

    def test_regex_key_any(self, doc):
        r = doc.select("[(regex)key]")
        assert _names(r) == ["secret"]

    def test_secret_regex_key(self, doc):
        r = doc.select("secret[(regex)key]")
        assert _names(r) == ["secret"]

    def test_secret_regex_key_contains(self, doc):
        r = doc.select('secret[(regex)key~="hs"]')
        assert _names(r) == ["secret"]

    def test_wrong_type_backend_u32_port(self, doc):
        r = doc.select("backend[(u32)port]")
        assert r == []

    def test_wrong_type_secret_jwt_key(self, doc):
...
class TestArgumentBasic:
    def test_server_arg0_exists(self, doc):
        r = doc.select("server[0]")
        assert _first_args(r) == ["primary", "replica"]

    def test_server_arg0_primary(self, doc):
        r = doc.select('server[0="primary"]')
        assert _first_args(r) == ["primary"]

    def test_server_arg0_replica(self, doc):
        r = doc.select('server[0="replica"]')
        assert _first_args(r) == ["replica"]

    def test_route_arg0_get(self, doc):
        r = doc.select('route[0="GET"]')
        assert _first_args(r) == ["GET", "GET", "GET"]

    def test_route_arg1_api_users(self, doc):
        r = doc.select('route[1="/api/users"]')
        assert _first_args(r) == ["GET", "POST"]

    def test_route_arg1_starts_api(self, doc):
        r = doc.select('route[1^="/api"]')
        assert _first_args(r) == ["GET", "POST", "GET"]

    def test_route_arg1_starts_static(self, doc):
        r = doc.select('route[1^="/static"]')
        assert _first_args(r) == ["GET"]

    def test_route_arg1_ends_users(self, doc):
        r = doc.select('route[1$="/users"]')
        assert _first_args(r) == ["GET", "POST"]

    def test_route_arg1_contains_health(self, doc):
        r = doc.select('route[1~="health"]')
        assert _first_args(r) == ["GET"]

    def test_route_any_arg_post(self, doc):
        r = doc.select('route[*="POST"]')
        assert _first_args(r) == ["POST"]

    def test_plugin_arg0_exists(self, doc):
        r = doc.select("plugin[0]")
        assert _first_args(r) == ["auth", "cache", "debug"]

    def test_plugin_arg1_exists(self, doc):
...
class TestArgumentTypeAnnotation:
    def test_expires_i32_arg0_exists(self, doc):
        r = doc.select("expires[(i32)0]")
        assert _names(r) == ["expires"]

    def test_expires_i32_arg0_value(self, doc):
        r = doc.select("expires[(i32)0=3600]")
        assert _names(r) == ["expires"]

    def test_wrong_type_expires_u32(self, doc):
        r = doc.select("expires[(u32)0]")
        assert r == []

    def test_i32_workers_i32_arg0(self, doc):
        r = doc.select("(i32)workers[(i32)0]")
        assert r == []

    def test_wildcard_i32_arg0(self, doc):
        r = doc.select("*[(i32)0]")
        assert _names(r) == ["expires"]
...
class TestCombinators:
    ...
    def test_nested_child(self, doc):
        r = doc.select("app > router > route")
        assert _first_args(r) == ["GET", "POST", "GET", "GET"]

    def test_child_no_match(self, doc):
        r = doc.select("app > route")
        assert r == []

    def test_plugins_child_plugin(self, doc):
        r = doc.select("plugins > plugin")
        assert _first_args(r) == ["auth", "cache", "debug"]

    def test_plugin_child_backend(self, doc):
        r = doc.select("plugin > backend")
        assert _names(r) == ["backend"]

    def test_typed_server_tls_true_child_host(self, doc):
        r = doc.select("(network)server[tls=#true] > host")
        assert _first_args(r) == ["localhost", "127.0.0.1"]

    def test_typed_server_tls_false_child_host(self, doc):
    ...
    def test_descendant_route(self, doc):
        r = doc.select("app route")
        assert _first_args(r) == ["GET", "POST", "GET", "GET"]

    def test_descendant_plugin_backend(self, doc):
...
class TestSiblings:
    def test_adjacent_get_plus_route(self, doc):
        r = doc.select('route[0="GET"] + route')
        assert _first_args(r) == ["POST", "GET"]

    def test_adjacent_post_plus_route(self, doc):
        r = doc.select('route[0="POST"] + route')
        assert _first_args(r) == ["GET"]

    def test_general_get_tilde_route(self, doc):
        r = doc.select('route[0="GET"][1$="/users"] ~ route')
        assert _first_args(r) == ["POST", "GET", "GET"]

    def test_adjacent_auth_plus_plugin(self, doc):
        r = doc.select('plugin[0="auth"] + plugin')
        assert _first_args(r) == ["cache"]

    def test_general_auth_tilde_plugin(self, doc):
        r = doc.select('plugin[0="auth"] ~ plugin')
        assert _first_args(r) == ["cache", "debug"]

    def test_adjacent_debug_plus_plugin(self, doc):
...
class TestCombined:
    def test_type_prop_descendant(self, doc):
        r = doc.select("(network)server[tls=#true] host")
        assert _first_args(r) == ["localhost", "127.0.0.1"]

    def test_child_wildcard_u32(self, doc):
        r = doc.select("app > *[(u32)max-conn]")
        assert _names(r) == ["limits"]

    def test_wildcard_i32_arg_empty(self, doc):
        r = doc.select("*[(i32)0]:empty")
        assert _names(r) == ["expires"]

    def test_multi_attr_filter(self, doc):
        r = doc.select('route[auth=#false][0="GET"]')
        assert _first_args(r) == ["GET", "GET"]

    def test_jwt_secret_regex_key(self, doc):
        r = doc.select("(jwt)secret[(regex)key]")
        assert _first_args(r) == ["hs256"]

    def test_descendant_plugin_u16_port_empty(self, doc):
        r = doc.select("plugins plugin[(u16)port]")
        assert r == []

    def test_descendant_app_backend_u16(self, doc):
...
class TestNot:
    def test_server_not_tls(self, doc):
        r = doc.select("server:not([tls])")
        assert r == []

    def test_server_not_port_8080(self, doc):
        r = doc.select("server:not([port=8080])")
        assert _first_args(r) == ["replica"]

    def test_route_not_auth_true(self, doc):
        r = doc.select("route:not([auth=#true])")
        assert _first_args(r) == ["GET", "GET"]

    def test_plugin_not_empty(self, doc):
        r = doc.select("plugin:not(:empty)")
        assert _first_args(r) == ["auth", "cache"]

    def test_wildcard_not_app_root(self, doc):
        r = doc.select("*:not(app):root")
        assert r == []

    def test_node_not_type_annotation(self, doc):
    ...
    def test_not_combined_with_filter(self, doc):
...
class TestHas:
    def test_plugin_has_backend(self, doc):
        r = doc.select("plugin:has(backend)")
        assert _first_args(r) == ["cache"]

    def test_server_has_host(self, doc):
        r = doc.select("server:has(host)")
        assert _first_args(r) == ["primary", "replica"]

    def test_app_has_child_router(self, doc):
        r = doc.select("app:has(> router)")
        assert _names(r) == ["app"]

    def test_plugin_has_child_secret(self, doc):
        r = doc.select("plugin:has(> secret)")
        assert _first_args(r) == ["auth"]

    def test_plugin_has_child_secret_regex_key(self, doc):
        r = doc.select("plugin:has(> secret[(regex)key])")
        assert _first_args(r) == ["auth"]

    def test_server_not_has_host(self, doc):
        r = doc.select("server:not(:has(host))")
        assert r == []

    def test_app_has_descendant_backend(self, doc):
        r = doc.select("app:has(backend)")
        assert _names(r) == ["app"]

    def test_plugin_has_child_expires(self, doc):
        r = doc.select("plugin:has(> expires)")
        assert _first_args(r) == ["auth"]

    def test_server_has_timeout(self, doc):
        r = doc.select("server:has(timeout)")
        assert _first_args(r) == ["primary", "replica"]

    def test_router_has_child_route_auth(self, doc):
...
class TestSelectOne:
    def test_returns_first(self, doc):
        node = doc.select_one("server")
        assert node is not None
        assert node.get_arg(0) == "primary"

    def test_returns_none(self, doc):
    ...
    def test_with_filter(self, doc):
        node = doc.select_one("server[tls=#true]")
        assert node is not None
        assert node.get_arg(0) == "primary"

    def test_comma_returns_first_document_order(self, doc):
    ...
    def test_no_match_comma(self, doc):
